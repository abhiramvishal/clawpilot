...
import type { CloudUserInfo, AuthState } from "@clawpilot/types"
import { CloudService } from "@clawpilot/cloud"
import { TelemetryService, PostHogTelemetryClient } from "@clawpilot/telemetry"
import { customToolRegistry } from "@clawpilot/core"
...
/**
 * Check if we should auto-open the ClawPilot sidebar after switching to a worktree.
...
async function checkWorktreeAutoOpen(
	...
	try {
		...
		if (normalizePath(currentPath) === normalizePath(worktreeAutoOpenPath)) {
			...
			outputChannel.appendLine(`[Worktree] Auto-opening ClawPilot sidebar for worktree: ${worktreeAutoOpenPath}`)

			// Open the ClawPilot sidebar with a slight delay to ensure UI is ready
			setTimeout(async () => {
				try {
					await vscode.commands.executeCommand("clawpilot.plusButtonClicked")
				} catch (error) {
...
export async function activate(context: vscode.ExtensionContext) {
	...
	// Initialize the provider *before* the ClawPilot Cloud service.
	const provider = new ClineProvider(context, outputChannel, "sidebar", contextProxy, mdmService)

	// Initialize ClawPilot Cloud service.
	...
	if (process.env.NODE_ENV === "development") {
		const watchPaths = [
			...
			{ path: path.join(context.extensionPath, "node_modules/@clawpilot/cloud"), pattern: "**/*" },
		]
