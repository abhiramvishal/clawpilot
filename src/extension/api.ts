...
import { IpcServer } from "@clawpilot/ipc"
...
export class API extends EventEmitter<ClawPilotEvents> implements ClawPilotAPI {
	...
	constructor(
		...
		if (enableLogging) {
			...
			this.logfile = path.join(os.tmpdir(), "clawpilot-messages.log")
		} else {
			this.log = () => {}
		}
		...
		if (socketPath) {
			...
			ipc.on(IpcMessageType.TaskCommand, async (clientId, command) => {
				const sendResponse = (eventName: ClawPilotEventName, payload: unknown[]) => {
					ipc.send(clientId, {
				...
				switch (command.commandName) {
					...
					case TaskCommandName.GetCommands:
						try {
							...
							sendResponse(ClawPilotEventName.CommandsResponse, [
								commands.map((cmd) => ({
									name: cmd.name,
									source: cmd.source,
					...
					case TaskCommandName.GetModes:
						try {
							...
							sendResponse(ClawPilotEventName.ModesResponse, [modes])
						} catch (error) {
							sendResponse(ClawPilotEventName.ModesResponse, [[]])
						}
					...
					case TaskCommandName.GetModels:
						try {
							...
							sendResponse(ClawPilotEventName.ModelsResponse, [models])
						} catch (error) {
							sendResponse(ClawPilotEventName.ModelsResponse, [{}])
						}
	...
	public override emit<K extends keyof ClawPilotEvents>(
		eventName: K,
		...args: K extends keyof ClawPilotEvents ? ClawPilotEvents[K] : never
	) {
		const data = { eventName: eventName as ClawPilotEventName, payload: args } as TaskEvent
	...
	private registerListeners(provider: ClineProvider) {
		provider.on(ClawPilotEventName.TaskCreated, (task) => {
			// Task Lifecycle

			task.on(ClawPilotEventName.TaskStarted, async () => {
				this.emit(ClawPilotEventName.TaskStarted, task.taskId)
				await this.fileLog(`[${new Date().toISOString()}] taskStarted -> ${task.taskId}\n`)
			})

			task.on(ClawPilotEventName.TaskCompleted, async (_, tokenUsage, toolUsage) => {
				this.emit(ClawPilotEventName.TaskCompleted, task.taskId, tokenUsage, toolUsage, {
			...
			task.on(ClawPilotEventName.TaskAborted, () => {
				this.emit(ClawPilotEventName.TaskAborted, task.taskId)
			})

			task.on(ClawPilotEventName.TaskFocused, () => {
				this.emit(ClawPilotEventName.TaskFocused, task.taskId)
			})

			task.on(ClawPilotEventName.TaskUnfocused, () => {
				this.emit(ClawPilotEventName.TaskUnfocused, task.taskId)
			})

			task.on(ClawPilotEventName.TaskActive, () => {
				this.emit(ClawPilotEventName.TaskActive, task.taskId)
			})

			task.on(ClawPilotEventName.TaskInteractive, () => {
				this.emit(ClawPilotEventName.TaskInteractive, task.taskId)
			})

			task.on(ClawPilotEventName.TaskResumable, () => {
				this.emit(ClawPilotEventName.TaskResumable, task.taskId)
			})

			task.on(ClawPilotEventName.TaskIdle, () => {
				this.emit(ClawPilotEventName.TaskIdle, task.taskId)
			})

			// Subtask Lifecycle

			task.on(ClawPilotEventName.TaskPaused, () => {
				this.emit(ClawPilotEventName.TaskPaused, task.taskId)
			})

			task.on(ClawPilotEventName.TaskUnpaused, () => {
				this.emit(ClawPilotEventName.TaskUnpaused, task.taskId)
			})

			task.on(ClawPilotEventName.TaskSpawned, (childTaskId) => {
				this.emit(ClawPilotEventName.TaskSpawned, task.taskId, childTaskId)
			})

			task.on(ClawPilotEventName.TaskDelegated as any, (childTaskId: string) => {
				;(this.emit as any)(ClawPilotEventName.TaskDelegated, task.taskId, childTaskId)
			})

			task.on(ClawPilotEventName.TaskDelegationCompleted as any, (childTaskId: string, summary: string) => {
				;(this.emit as any)(ClawPilotEventName.TaskDelegationCompleted, task.taskId, childTaskId, summary)
			})

			task.on(ClawPilotEventName.TaskDelegationResumed as any, (childTaskId: string) => {
				;(this.emit as any)(ClawPilotEventName.TaskDelegationResumed, task.taskId, childTaskId)
			})

			// Task Execution

			task.on(ClawPilotEventName.Message, async (message) => {
				this.emit(ClawPilotEventName.Message, { taskId: task.taskId, ...message })
			...
			task.on(ClawPilotEventName.TaskModeSwitched, (taskId, mode) => {
				this.emit(ClawPilotEventName.TaskModeSwitched, taskId, mode)
			})

			task.on(ClawPilotEventName.TaskAskResponded, () => {
				this.emit(ClawPilotEventName.TaskAskResponded, task.taskId)
			})

			task.on(ClawPilotEventName.QueuedMessagesUpdated, (taskId, messages) => {
				this.emit(ClawPilotEventName.QueuedMessagesUpdated, taskId, messages)
			})

			// Task Analytics

			task.on(ClawPilotEventName.TaskToolFailed, (taskId, tool, error) => {
				this.emit(ClawPilotEventName.TaskToolFailed, taskId, tool, error)
			})

			task.on(ClawPilotEventName.TaskTokenUsageUpdated, (_, tokenUsage, toolUsage) => {
				this.emit(ClawPilotEventName.TaskTokenUsageUpdated, task.taskId, tokenUsage, toolUsage)
			})

			// Let's go!

			this.emit(ClawPilotEventName.TaskCreated, task.taskId)
		})
	}
	...
	public getConfiguration(): ClawPilotSettings {
		return Object.fromEntries(
			Object.entries(this.sidebarProvider.getValues()).filter(([key]) => !isSecretStateKey(key)),
		)
	}

	public async setConfiguration(values: ClawPilotSettings) {
