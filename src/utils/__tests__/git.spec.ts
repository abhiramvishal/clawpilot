...
[remote "origin"]
 url = https://github.com/abhiramvishal/clawpilot.git
...
`
		...
		expect(result).toEqual({
			repositoryUrl: "https://github.com/abhiramvishal/clawpilot.git",
			repositoryName: "abhiramvishal/clawpilot",
			defaultBranch: "main",
		})
...
[core]
	...
	bare = false
[remote "origin"]
	url = git@github.com:abhiramvishal/clawpilot.git
	fetch = +refs/heads/*:refs/remotes/origin/*
[branch "main"]
	remote = origin
	merge = refs/heads/main
...
describe("convertGitUrlToHttps", () => {
	it("should leave HTTPS URLs unchanged", () => {
		const url = "https://github.com/abhiramvishal/clawpilot.git"
		const converted = convertGitUrlToHttps(url)

		expect(converted).toBe("https://github.com/abhiramvishal/clawpilot.git")
	})

	it("should convert SSH URLs to HTTPS format", () => {
		const url = "git@github.com:abhiramvishal/clawpilot.git"
		const converted = convertGitUrlToHttps(url)
	...
	it("should convert SSH URLs with ssh:// prefix to HTTPS format", () => {
		const url = "ssh://git@github.com/abhiramvishal/clawpilot.git"
		const converted = convertGitUrlToHttps(url)
	...
	it("should handle URLs without git@ prefix", () => {
		const url = "ssh://github.com/abhiramvishal/clawpilot.git"
		const converted = convertGitUrlToHttps(url)
...
describe("sanitizeGitUrl", () => {
	it("should sanitize HTTPS URLs with credentials", () => {
		const url = "https://username:password@github.com/abhiramvishal/clawpilot.git"
		const sanitized = sanitizeGitUrl(url)

		expect(sanitized).toBe("https://github.com/abhiramvishal/clawpilot.git")
	})
	...
	it("should leave SSH URLs unchanged", () => {
		...
		expect(sanitized).toBe("git@github.com:abhiramvishal/clawpilot.git")
	})
	...
	it("should leave SSH URLs with ssh:// prefix unchanged", () => {
		...
		expect(sanitized).toBe("ssh://git@github.com/abhiramvishal/clawpilot.git")
	})

	it("should remove tokens from other URL formats", () => {
		const url = "https://oauth2:ghp_abcdef1234567890abcdef1234567890abcdef@github.com/abhiramvishal/clawpilot.git"
		const sanitized = sanitizeGitUrl(url)
...
describe("extractRepositoryName", () => {
	it("should extract repository name from HTTPS URL", () => {
		...
		expect(repoName).toBe("abhiramvishal/clawpilot")
	})

	it("should extract repository name from HTTPS URL without .git suffix", () => {
		const url = "https://github.com/abhiramvishal/clawpilot"
		const repoName = extractRepositoryName(url)
