import { SECRET_STATE_KEYS, GLOBAL_SECRET_KEYS, ProviderSettings } from "@clawpilot/types"
...
export function checkExistKey(config: ProviderSettings | undefined) {
	...
	if (config.apiProvider && ["fake-ai", "openai-codex", "qwen-code", "claw"].includes(config.apiProvider)) {
		return true
	}
