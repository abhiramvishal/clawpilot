...
export class MdmService {
	...
	private getMdmConfigPath(): string {
		...
		switch (platform) {
			case "win32": {
				// Windows: %ProgramData%\ClawPilot\mdm.json or mdm.dev.json
				const programData = process.env.PROGRAMDATA || "C:\\ProgramData"
				return path.join(programData, "ClawPilot", configFileName)
			}

			case "darwin":
				// macOS: /Library/Application Support/ClawPilot/mdm.json or mdm.dev.json
				return `/Library/Application Support/ClawPilot/${configFileName}`

			case "linux":
			default:
				// Linux: /etc/clawpilot/mdm.json or mdm.dev.json
				return `/etc/clawpilot/${configFileName}`
		}
	}
