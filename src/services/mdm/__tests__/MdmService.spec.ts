...
vi.mock("@clawpilot/cloud", () => ({
	...
	PRODUCTION_CLERK_BASE_URL: "https://clerk.clawpilot.com",
}))
	...
	t: vi.fn((key: string) => {
		const translations: Record<string, string> = {
			"mdm.errors.cloud_auth_required":
				"Your organization requires ClawPilot Cloud authentication. Please sign in to continue.",
			"mdm.errors.organization_mismatch":
				"You must be authenticated with your organization's ClawPilot Cloud account.",
...
import { CloudService, getClerkBaseUrl, PRODUCTION_CLERK_BASE_URL } from "@clawpilot/cloud"
...
describe("MdmService", () => {
	...
	beforeEach(() => {
		...
		mockGetClerkBaseUrl.mockReturnValue("https://dev.clerk.clawpilot.com")
	...
	describe("platform-specific config paths", () => {
		...
		it("should use correct path for Windows in production", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith(path.join("C:\\ProgramData", "ClawPilot", "mdm.json"))
		})
		...
		it("should use correct path for Windows in development", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith(path.join("C:\\ProgramData", "ClawPilot", "mdm.dev.json"))
		})
		...
		it("should use correct path for macOS in production", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith("/Library/Application Support/ClawPilot/mdm.json")
		})
		...
		it("should use correct path for macOS in development", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith("/Library/Application Support/ClawPilot/mdm.dev.json")
		})
		...
		it("should use correct path for Linux in production", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith("/etc/clawpilot/mdm.json")
		})
		...
		it("should use correct path for Linux in development", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith("/etc/clawpilot/mdm.dev.json")
		})
	...
	describe("compliance checking", () => {
		...
		it("should be non-compliant when not authenticated", async () => {
			...
			if (!compliance.compliant) {
				expect(compliance.reason).toContain("Your organization requires ClawPilot Cloud authentication")
			}
		})
