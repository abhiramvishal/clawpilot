...
// Global claw directory - computed once
const GLOBAL_CLAW_DIR = p(HOME_DIR, ".claw")
const GLOBAL_AGENTS_DIR = p(HOME_DIR, ".agents")

// Mock claw-config
vi.mock("../../claw-config", () => ({
	getGlobalClawDirectory: () => GLOBAL_CLAW_DIR,
...
describe("SkillsManager", () => {
	...
	const globalSkillsDir = p(GLOBAL_CLAW_DIR, "skills")
	const globalSkillsCodeDir = p(GLOBAL_CLAW_DIR, "skills-code")
	const globalSkillsArchitectDir = p(GLOBAL_CLAW_DIR, "skills-architect")
	const projectClawDir = p(PROJECT_DIR, ".claw")
	const projectSkillsDir = p(projectClawDir, "skills")
...
# Too Long Description`)
		...
		it("should handle symlinked skills directory", async () => {
			...
			// Simulate .claw/skills being a symlink to /shared/skills
			mockDirectoryExists.mockImplementation(async (dir: string) => {
				return dir === globalSkillsDir
			})
...
Instructions here...`
		...
		it("should handle symlinked skill subdirectory", async () => {
			...
			// Simulate .claw/skills/my-alias being a symlink to /external/actual-skill
			mockDirectoryExists.mockImplementation(async (dir: string) => {
				return dir === globalSkillsDir
			})
...
Instructions here...`
		...
		it("should prioritize .claw skills over .agents skills with same name", async () => {
...
# Agent Common Skill`
				...
				if (file === rooSkillMd) {
					return `---
name: common-skill
description: Claw version (should take priority)
---
...
# Claw Common Skill`
			...
			// .claw should override .agents
			expect(commonSkill?.description).toBe("Claw version (should take priority)")
		})
...
Instructions`)
	...
	describe("createSkill", () => {
		it("should create a new global skill", async () => {
			...
			expect(createdPath).toBe(p(GLOBAL_CLAW_DIR, "skills", "new-skill", "SKILL.md"))
			expect(mockMkdir).toHaveBeenCalledWith(p(GLOBAL_CLAW_DIR, "skills", "new-skill"), { recursive: true })
			expect(mockWriteFile).toHaveBeenCalled()

			// Verify the content written
			const writeCall = mockWriteFile.mock.calls[0]
			expect(writeCall[0]).toBe(p(GLOBAL_CLAW_DIR, "skills", "new-skill", "SKILL.md"))
		...
		it("should create a mode-specific skill with modeSlugs array", async () => {
			...
			expect(createdPath).toBe(p(GLOBAL_CLAW_DIR, "skills", "code-skill", "SKILL.md"))
		...
		it("should create a project skill", async () => {
			...
			expect(createdPath).toBe(p(PROJECT_DIR, ".claw", "skills", "project-skill", "SKILL.md"))
		})
...
Instructions`)
	...
	describe("moveSkill", () => {
		it("should move a skill from generic to mode-specific directory", async () => {
			...
			const destDir = p(GLOBAL_CLAW_DIR, "skills-code", "test-skill")
			const destSkillsDir = p(GLOBAL_CLAW_DIR, "skills-code")
...
Instructions`)
		...
		it("should move a skill from one mode to another", async () => {
			const sourceSkillsDir = p(GLOBAL_CLAW_DIR, "skills-code")
			const sourceDir = p(sourceSkillsDir, "test-skill")
			const testSkillMd = p(sourceDir, "SKILL.md")
			const destDir = p(GLOBAL_CLAW_DIR, "skills-architect", "test-skill")
			const destSkillsDir = p(GLOBAL_CLAW_DIR, "skills-architect")
