...
import { rooDefaultModelId, getApiProtocol, type ImageGenerationApiMethod } from "@clawpilot/types"
...
export class ClawHandler extends BaseOpenAiCompatibleProvider<string> {
	...
	override async *createMessage(
		...
		try {
			...
			if (lastUsage) {
				...
				const apiProtocol = getApiProtocol("claw", modelId)
	...
	override getModel() {
		...
		const models = getModelsFromCache("claw") || {}
	...
	/**
	 * Generate an image using ClawPilot Cloud's image generation API
