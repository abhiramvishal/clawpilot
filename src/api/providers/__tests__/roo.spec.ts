...
vitest.mock("../../../i18n", () => ({
	t: vitest.fn((key: string) => {
		if (key === "common:errors.roo.authenticationRequired") {
			return "Authentication required for ClawPilot Cloud"
		}
		return key
	}),
}))
...
describe("ClawHandler", () => {
	...
	describe("constructor", () => {
		...
		it("should not throw error if CloudService is not available", () => {
			...
			expect(() => {
				new ClawHandler(mockOptions)
			}).not.toThrow()
			// Constructor should succeed even without CloudService
			const handler = new ClawHandler(mockOptions)
			expect(handler).toBeInstanceOf(ClawHandler)
		})
			...
			expect(mockCreate).toHaveBeenCalledWith(
				...
				expect.objectContaining({
					headers: expect.objectContaining({
						"X-Claw-App-Version": expect.any(String),
					}),
				}),
			)
		})
	...
	describe("completePrompt", () => {
		...
		it("should handle API errors", async () => {
			...
			await expect(handler.completePrompt("Test prompt")).rejects.toThrow(
				"ClawPilot Cloud completion error: API Error",
			)
		})
	...
	describe("getModel", () => {
		...
		it("should return default model when no model specified", () => {
			const handlerWithoutModel = new ClawHandler({})
		...
		it("should handle unknown model ID with fallback info", () => {
			const handlerWithUnknownModel = new ClawHandler({
		...
		it("should handle any model ID since models are loaded dynamically", () => {
			...
			for (const modelId of testModelIds) {
				const handlerWithModel = new ClawHandler({ apiModelId: modelId })
		...
		it("should return cached model info with settings applied from API", () => {
			const handlerWithMinimax = new ClawHandler({
