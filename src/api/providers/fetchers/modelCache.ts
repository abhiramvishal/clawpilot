...
import type { ProviderName, ModelRecord } from "@clawpilot/types"
import { modelInfoSchema, TelemetryEventName } from "@clawpilot/types"
...
async function fetchModelsFromProvider(options: GetModelsOptions): Promise<ModelRecord> {
	...
	switch (provider) {
		...
		case "roo": {
			...
			models = await getClawModels(rooBaseUrl, options.apiKey)
			break
		}
		default: {
