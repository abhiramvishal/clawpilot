...
/**
 * Fetches available models from the ClawPilot Cloud provider
 *
 * @param baseUrl The base URL of the ClawPilot Cloud provider
 * @param apiKey The API key (session token) for the ClawPilot Cloud provider
	...
	try {
		...
		try {
			...
			if (!parsed.success) {
				...
				throw new Error("Failed to fetch ClawPilot Cloud models: Unexpected response format.")
			}
		...
		if (error.name === "AbortError") {
			throw new Error("Failed to fetch ClawPilot Cloud models: Request timed out after 10 seconds.")
		}

		// Handle fetch errors
		if (error.message?.includes("HTTP")) {
			throw new Error(`Failed to fetch ClawPilot Cloud models: ${error.message}. Check base URL and API key.`)
		}

		// Handle network errors
		if (error instanceof TypeError) {
			throw new Error(
				"Failed to fetch ClawPilot Cloud models: No response from server. Check ClawPilot Cloud server status and base URL.",
			)
		}
