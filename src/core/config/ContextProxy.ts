...
type ClawPilotSettingsKey = keyof ClawPilotSettings
...
export class ContextProxy {
	 ...
	 */
	private sanitizeProviderValues(values: ClawPilotSettings): ClawPilotSettings {
		...
		for (const key of legacyKeys) {
			if (key in sanitizedValues) {
				...
				sanitizedValues = copy as ClawPilotSettings
			}
		}
		...
		if (values.apiProvider !== undefined && !isKnownProvider) {
			...
			return restValues as ClawPilotSettings
		}
	...
	public async setValue<K extends ClawPilotSettingsKey>(key: K, value: ClawPilotSettings[K]) {
	...
	public getValue<K extends ClawPilotSettingsKey>(key: K): ClawPilotSettings[K] {
		return isSecretStateKey(key)
			? (this.getSecret(key as SecretStateKey) as ClawPilotSettings[K])
			: (this.getGlobalState(key as GlobalStateKey) as ClawPilotSettings[K])
	}

	public getValues(): ClawPilotSettings {
	...
	public async setValues(values: ClawPilotSettings) {
		const entries = Object.entries(values) as [ClawPilotSettingsKey, unknown][]
