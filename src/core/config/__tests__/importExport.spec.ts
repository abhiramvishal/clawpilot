...
import type { ProviderName } from "@clawpilot/types"
	...
	describe("exportSettings", () => {
		...
		it("should export settings to the selected file location", async () => {
			;(vscode.window.showSaveDialog as Mock).mockResolvedValue({
				fsPath: "/mock/path/clawpilot-settings.json",
			})
			...
			expect(safeWriteJson).toHaveBeenCalledWith("/mock/path/clawpilot-settings.json", {
		...
		it("should use the correct default save location", async () => {
			...
			expect(vscode.Uri.file).toHaveBeenCalledWith(path.join("/mock/home", "Downloads", "clawpilot-settings.json"))
		})
