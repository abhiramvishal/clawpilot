...
import { TelemetryService } from "@clawpilot/telemetry"
import { CloudService, getClawPilotApiUrl } from "@clawpilot/cloud"
...
import type { ClineMessage, TodoItem } from "@clawpilot/types"
	...
	implements vscode.WebviewViewProvider, TelemetryPropertiesProvider, TaskProviderLike
{
	...
	constructor(
		...
		this.taskCreationCallback = (instance: Task) => {
			this.emit(ClawPilotEventName.TaskCreated, instance)

			// Create named listener functions so we can remove them later.
			const onTaskStarted = () => this.emit(ClawPilotEventName.TaskStarted, instance.taskId)
			const onTaskCompleted = (taskId: string, tokenUsage: TokenUsage, toolUsage: ToolUsage) =>
				this.emit(ClawPilotEventName.TaskCompleted, taskId, tokenUsage, toolUsage)
			const onTaskAborted = async () => {
				this.emit(ClawPilotEventName.TaskAborted, instance.taskId)
			...
			const onTaskFocused = () => this.emit(ClawPilotEventName.TaskFocused, instance.taskId)
			const onTaskUnfocused = () => this.emit(ClawPilotEventName.TaskUnfocused, instance.taskId)
			const onTaskActive = (taskId: string) => this.emit(ClawPilotEventName.TaskActive, taskId)
			const onTaskInteractive = (taskId: string) => this.emit(ClawPilotEventName.TaskInteractive, taskId)
			const onTaskResumable = (taskId: string) => this.emit(ClawPilotEventName.TaskResumable, taskId)
			const onTaskIdle = (taskId: string) => this.emit(ClawPilotEventName.TaskIdle, taskId)
			const onTaskPaused = (taskId: string) => this.emit(ClawPilotEventName.TaskPaused, taskId)
			const onTaskUnpaused = (taskId: string) => this.emit(ClawPilotEventName.TaskUnpaused, taskId)
			const onTaskSpawned = (taskId: string) => this.emit(ClawPilotEventName.TaskSpawned, taskId)
			const onTaskUserMessage = (taskId: string) => this.emit(ClawPilotEventName.TaskUserMessage, taskId)
			const onTaskTokenUsageUpdated = (taskId: string, tokenUsage: TokenUsage, toolUsage: ToolUsage) =>
				this.emit(ClawPilotEventName.TaskTokenUsageUpdated, taskId, tokenUsage, toolUsage)

			// Attach the listeners.
			instance.on(ClawPilotEventName.TaskStarted, onTaskStarted)
			instance.on(ClawPilotEventName.TaskCompleted, onTaskCompleted)
			instance.on(ClawPilotEventName.TaskAborted, onTaskAborted)
			instance.on(ClawPilotEventName.TaskFocused, onTaskFocused)
			instance.on(ClawPilotEventName.TaskUnfocused, onTaskUnfocused)
			instance.on(ClawPilotEventName.TaskActive, onTaskActive)
			instance.on(ClawPilotEventName.TaskInteractive, onTaskInteractive)
			instance.on(ClawPilotEventName.TaskResumable, onTaskResumable)
			instance.on(ClawPilotEventName.TaskIdle, onTaskIdle)
			instance.on(ClawPilotEventName.TaskPaused, onTaskPaused)
			instance.on(ClawPilotEventName.TaskUnpaused, onTaskUnpaused)
			instance.on(ClawPilotEventName.TaskSpawned, onTaskSpawned)
			instance.on(ClawPilotEventName.TaskUserMessage, onTaskUserMessage)
			instance.on(ClawPilotEventName.TaskTokenUsageUpdated, onTaskTokenUsageUpdated)

			// Store the cleanup functions for later removal.
			this.taskEventListeners.set(instance, [
				() => instance.off(ClawPilotEventName.TaskStarted, onTaskStarted),
				() => instance.off(ClawPilotEventName.TaskCompleted, onTaskCompleted),
				() => instance.off(ClawPilotEventName.TaskAborted, onTaskAborted),
				() => instance.off(ClawPilotEventName.TaskFocused, onTaskFocused),
				() => instance.off(ClawPilotEventName.TaskUnfocused, onTaskUnfocused),
				() => instance.off(ClawPilotEventName.TaskActive, onTaskActive),
				() => instance.off(ClawPilotEventName.TaskInteractive, onTaskInteractive),
				() => instance.off(ClawPilotEventName.TaskResumable, onTaskResumable),
				() => instance.off(ClawPilotEventName.TaskIdle, onTaskIdle),
				() => instance.off(ClawPilotEventName.TaskUserMessage, onTaskUserMessage),
				() => instance.off(ClawPilotEventName.TaskPaused, onTaskPaused),
				() => instance.off(ClawPilotEventName.TaskUnpaused, onTaskUnpaused),
				() => instance.off(ClawPilotEventName.TaskSpawned, onTaskSpawned),
				() => instance.off(ClawPilotEventName.TaskTokenUsageUpdated, onTaskTokenUsageUpdated),
			])
		}

		// Initialize ClawPilot Cloud profile sync.
	...
	async addClineToStack(task: Task) {
		...
		task.emit(ClawPilotEventName.TaskFocused)
	...
	async removeClineFromStack(options?: { skipDelegationRepair?: boolean }) {
		...
		if (task) {
			...
			task.emit(ClawPilotEventName.TaskUnfocused)
	...
	*/
		...
		*/
        ...
        <html lang="en">
          <head>
            ...
            <meta http-equiv="Content-Security-Policy" content="default-src 'none'; font-src ${webview.cspSource} data:; style-src ${webview.cspSource} 'unsafe-inline'; img-src ${webview.cspSource} https://storage.googleapis.com https://img.clerk.com data:; media-src ${webview.cspSource}; script-src ${webview.cspSource} 'wasm-unsafe-eval' 'nonce-${nonce}' https://ph.clawpilot.com 'strict-dynamic'; connect-src ${webview.cspSource} ${openRouterDomain} https://api.requesty.ai https://ph.clawpilot.com;">
            <link rel="stylesheet" type="text/css" href="${stylesUri}">
			<link href="${codiconsUri}" rel="stylesheet" />
			<script nonce="${nonce}">
				window.IMAGES_BASE_URI = "${imagesUri}"
	...
	public async handleModeSwitch(newMode: Mode) {
		...
		if (task) {
			...
			task.emit(ClawPilotEventName.TaskModeSwitched, task.taskId, newMode)
		...
		this.emit(ClawPilotEventName.ModeChanged, newMode)
	...
	async activateProviderProfile(
		...
		if (providerSettings.apiProvider) {
			this.emit(ClawPilotEventName.ProviderProfileChanged, { name, provider: providerSettings.apiProvider })
		}
	}
	...
	async ensureMcpServersDirectoryExists(): Promise<string> {
		...
		if (process.platform === "win32") {
			// Windows: %APPDATA%\clawpilot\MCP
			mcpServersDir = path.join(os.homedir(), "AppData", "Roaming", "clawpilot", "MCP")
		} else if (process.platform === "darwin") {
			// macOS: ~/Documents/Cline/MCP
			mcpServersDir = path.join(os.homedir(), "Documents", "Cline", "MCP")
		} else {
			// Linux: ~/.local/share/Cline/MCP
			mcpServersDir = path.join(os.homedir(), ".local", "share", "clawpilot", "MCP")
		}
		...
		try {
			...
			return path.join(os.homedir(), ".clawpilot", "mcp")
		}
	...
	async getStateToPostToWebview(): Promise<ExtensionState> {
		...
		return {
			...
			telemetryKey,
			machineId,
			showClawIgnoredFiles: showClawIgnoredFiles ?? false,
			...
			cloudAuthSkipModel: this.context.globalState.get<boolean>("claw-auth-skip-model") ?? false,
			cloudOrganizations,
			...
			cloudApiUrl: getClawPilotApiUrl(),
	...
	> {
		...
		return {
			...
			showClawIgnoredFiles: stateValues.showClawIgnoredFiles ?? false,
	...
	public async setValue<K extends keyof ClawPilotSettings>(key: K, value: ClawPilotSettings[K]) {
		await this.contextProxy.setValue(key, value)
	}

	public getValue<K extends keyof ClawPilotSettings>(key: K) {
		return this.contextProxy.getValue(key)
	}
	...
	public async setValues(values: ClawPilotSettings) {
	...
	public async createTask(
		...
		configuration: ClawPilotSettings = {},
	): Promise<Task> {
		if (configuration) {
	...
	public async delegateParentAndOpenChild(params: {
		...
		try {
			this.emit(ClawPilotEventName.TaskDelegated, parentTaskId, child.taskId)
		} catch {
			// non-fatal
		}
	...
	public async reopenParentFromDelegation(params: {
		...
		try {
			this.emit(ClawPilotEventName.TaskDelegationCompleted, parentTaskId, childTaskId, completionResultSummary)
		} catch {
			// non-fatal
		}
		...
		try {
			this.emit(ClawPilotEventName.TaskDelegationResumed, parentTaskId, childTaskId)
		} catch {
			// non-fatal
		}
	}
