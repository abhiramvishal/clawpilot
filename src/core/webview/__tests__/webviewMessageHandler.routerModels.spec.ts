...
describe("webviewMessageHandler - requestRouterModels provider filter", () => {
	...
	beforeEach(() => {
		...
		getModelsMock.mockImplementation(async (options: any) => {
			switch (options?.provider) {
				case "claw":
					return { "claw/sonnet": { contextWindow: 8192, supportsPromptCache: false } }
				case "openrouter":
	...
	it("fetches only requested provider when values.provider is present ('claw')", async () => {
		...
		// Only "claw" key should be present
		const keys = Object.keys(routerModels)
		expect(keys).toEqual(["claw"])
		expect(Object.keys(routerModels.claw || {})).toContain("claw/sonnet")

		// getModels should have been called exactly once for claw
		const providersCalled = getModelsMock.mock.calls.map((c: any[]) => c[0]?.provider)
		expect(providersCalled).toEqual(["claw"])
	})
	...
	it("defaults to aggregate fetching when no provider filter is sent", async () => {
		...
		expect(routerModels).toHaveProperty("claw")
