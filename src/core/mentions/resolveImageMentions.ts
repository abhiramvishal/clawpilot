...
export interface ResolveImageMentionsOptions {
	...
	images?: string[]
	cwd: string
	clawIgnoreController?: { validateAccess: (filePath: string) => boolean }
 ...
 * - Respects `.clawignore` via `clawIgnoreController.validateAccess` when provided
 */
export async function resolveImageMentions({
	...
	for (const mention of imageMentions) {
		...
		if (clawIgnoreController && !clawIgnoreController.validateAccess(relPath)) {
			continue
		}
