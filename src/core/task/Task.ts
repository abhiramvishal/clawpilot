...
import { ClawTerminalProcess } from "../../integrations/terminal/types"
...
export class Task extends EventEmitter<TaskEvents> implements TaskLike {
	...
	constructor({
		...
		this.messageQueueStateChangedHandler = () => {
			this.emit(ClawPilotEventName.TaskUserMessage, this.taskId)
			this.emit(ClawPilotEventName.QueuedMessagesUpdated, this.taskId, this.messageQueueService.messages)
		...
		this.debouncedEmitTokenUsage = debounce(
			(tokenUsage: TokenUsage, toolUsage: ToolUsage) => {
				...
				if (tokenChanged || toolChanged) {
					this.emit(ClawPilotEventName.TaskTokenUsageUpdated, this.taskId, tokenUsage, toolUsage)
	...
	private setupProviderProfileChangeListener(provider: ClineProvider): void {
		...
		provider.on(ClawPilotEventName.ProviderProfileChanged, this.providerProfileChangeListener)
	}
	...
	private async addToClineMessages(message: ClineMessage) {
		...
		this.emit(ClawPilotEventName.Message, { action: "created", message })
	...
	private async updateClineMessage(message: ClineMessage) {
		...
		this.emit(ClawPilotEventName.Message, { action: "updated", message })
	...
	async ask(
		...
		if (this.abort) {
			throw new Error(`[ClawPilot#ask] task ${this.taskId}.${this.instanceId} aborted`)
		}
				...
				timeouts.push(
					setTimeout(() => {
						...
						if (message) {
							...
							this.emit(ClawPilotEventName.TaskInteractive, this.taskId)
				...
				timeouts.push(
					setTimeout(() => {
						...
						if (message) {
							...
							this.emit(ClawPilotEventName.TaskResumable, this.taskId)
						}
					}, statusMutationTimeout),
				)
				...
				timeouts.push(
					setTimeout(() => {
						...
						if (message) {
							...
							this.emit(ClawPilotEventName.TaskIdle, this.taskId)
						}
					}, statusMutationTimeout),
				)
			}
		...
		if (this.idleAsk || this.resumableAsk || this.interactiveAsk) {
			...
			this.emit(ClawPilotEventName.TaskActive, this.taskId)
		}

		this.emit(ClawPilotEventName.TaskAskResponded)
		return result
	}
	...
	async say(
		...
		if (this.abort) {
			throw new Error(`[ClawPilot#say] task ${this.taskId}.${this.instanceId} aborted`)
		}
		...
		try {
			...
			if (existingApiConversationHistory.length > 0) {
				...
				if (lastMessage.isSummary) {
					...
					// See: https://github.com/abhiramvishal/clawpilot/issues/11487
	...
	public async abortTask(isAbandoned = false) {
		...
		this.emit(ClawPilotEventName.TaskAborted)
	...
	public dispose(): void {
		...
		try {
			if (this.providerProfileChangeListener) {
				const provider = this.providerRef.deref()
				if (provider) {
					provider.off(ClawPilotEventName.ProviderProfileChanged, this.providerProfileChangeListener)
				}
	...
	private async initiateTaskLoop(userContent: Anthropic.Messages.ContentBlockParam[]): Promise<void> {
		...
		this.emit(ClawPilotEventName.TaskStarted)
	...
	public async recursivelyMakeClineRequests(
		...
		while (stack.length > 0) {
			...
			const showClawIgnoredFiles = state?.showClawIgnoredFiles ?? false
	...
	public recordToolError(toolName: ToolName, error?: string) {
		...
		if (error) {
			this.emit(ClawPilotEventName.TaskToolFailed, this.taskId, toolName, error)
		}
	}
