import { ClawPilotEventName, ProviderSettings, TokenUsage, ToolUsage } from "@clawpilot/types"
...
vi.mock("../../protect/ClawProtectedController")
...
describe("Task token usage throttling", () => {
	...
	test("should emit TaskTokenUsageUpdated immediately on first change", async () => {
		...
		expect(emitSpy).toHaveBeenCalledWith(
			ClawPilotEventName.TaskTokenUsageUpdated,
			task.taskId,
			expect.any(Object),
			expect.any(Object),
		)
	...
	test("should throttle subsequent emissions within 2 seconds", async () => {
		...
		const firstEmitCount = emitSpy.mock.calls.filter(
			(call) => call[0] === ClawPilotEventName.TaskTokenUsageUpdated,
		).length
	...
	test("should force final emission on task abort", async () => {
		...
		const tokenUsageUpdateIndex = calls.findIndex((call) => call[0] === ClawPilotEventName.TaskTokenUsageUpdated)
		const taskAbortedIndex = calls.findIndex((call) => call[0] === ClawPilotEventName.TaskAborted)
