...
describe("newTaskTool", () => {
	...
	describe("VSCode setting: newTaskRequireTodos", () => {
		...
		it("should check VSCode setting with Package.name configuration key", async () => {
			...
			expect(mockGetConfiguration).toHaveBeenCalledWith("clawpilot")
			expect(mockGet).toHaveBeenCalledWith("newTaskRequireTodos", false)
		})

		it("should use current Package.name value (clawpilot-nightly) when accessing VSCode configuration", async () => {
			...
			;(pkg.Package as any).name = "clawpilot-nightly"
			...
			expect(mockGetConfiguration).toHaveBeenCalledWith("clawpilot-nightly")
			expect(mockGet).toHaveBeenCalledWith("newTaskRequireTodos", false)
		})
	})
