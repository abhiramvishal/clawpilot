...
import type { ToolName } from "@clawpilot/types"
...
vitest.mock("../../../i18n", () => ({
	t: vitest.fn((key, options) => {
		...
		if (key === "tools:toolRepetitionLimitReached" && options?.toolName) {
			return `Claw appears to be stuck in a loop, attempting the same action (${options.toolName}) repeatedly. This might indicate a problem with its current strategy. Consider rephrasing the task, providing more specific instructions, or guiding it towards a different approach.`
		}
		return key
	}),
}))
