...
import { CommandExecutionStatus, DEFAULT_TERMINAL_OUTPUT_PREVIEW_SIZE, PersistedCommandOutput } from "@clawpilot/types"
...
import { ExitCodeDetails, ClawTerminalCallbacks, ClawTerminalProcess } from "../../integrations/terminal/types"
...
export class ExecuteCommandTool extends BaseTool<"execute_command"> {
	...
	async execute(params: ExecuteCommandParams, task: Task, callbacks: ToolCallbacks): Promise<void> {
		...
		try {
			...
			const ignoredFileAttemptedToAccess = task.clawIgnoreController?.validateCommand(canonicalCommand)

			if (ignoredFileAttemptedToAccess) {
				await task.say("clawignore_error", ignoredFileAttemptedToAccess)
				pushToolResult(formatResponse.clawIgnoreError(ignoredFileAttemptedToAccess))
				return
			}
...
export async function executeCommandInTerminal(
	...
	const callbacks: ClawTerminalCallbacks = {
		onLine: async (lines: string, process: ClawTerminalProcess) => {
