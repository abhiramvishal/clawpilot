...
import { isLanguage } from "@clawpilot/types"
...
export async function loadRuleFiles(cwd: string, enableSubfolderRules: boolean = false): Promise<string> {
	...
	// Check for .claw/rules/ directories in order (global, project-local, and optionally subfolders)
	...
	// If we found rules in .claw/rules/ directories, return them
	if (rules.length > 0) {
		return "\n# Rules from .claw directories:\n\n" + rules.join("\n\n")
	}

	// Fall back to existing behavior for legacy .clawrules/.clinerules files
	const ruleFiles = [".clawrules", ".clinerules"]
...
/**
 * Load all AGENTS.md files from project root and optionally subdirectories with .claw folders
...
async function loadAllAgentRulesFiles(cwd: string, enableSubfolderRules: boolean = false): Promise<string> {
	...
	// When enabled, load from root and all subdirectories with .claw folders
...
export async function addCustomInstructions(
	...
	if (mode) {
		...
		// Check for .claw/rules-${mode}/ directories in order (global, project-local, and optionally subfolders)
		for (const rooDir of rooDirectories) {
		...
		// If we found mode-specific rules in .claw/rules-${mode}/ directories, use them
		if (modeRules.length > 0) {
			modeRuleContent = "\n" + modeRules.join("\n\n")
			usedRuleFile = `rules-${mode} directories`
		} else {
			// Fall back to existing behavior for legacy files
			const rooModeRuleFile = `.clawrules-${mode}`
	...
	if (modeRuleContent && modeRuleContent.trim()) {
		if (usedRuleFile.includes(path.join(".claw", `rules-${mode}`))) {
	...
	// Load from root and optionally subdirectories with .claw folders based on enableSubfolderRules setting
