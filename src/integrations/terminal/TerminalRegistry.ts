...
import { ClawTerminal, ClawTerminalProvider } from "./types"
...
export class TerminalRegistry {
	private static terminals: ClawTerminal[] = []
		...
		try {
			const startDisposable = vscode.window.onDidStartTerminalShellExecution?.(
				async (e: vscode.TerminalShellExecutionStartEvent) => {
					...
					if (terminal) {
						...
						console.error(
							"[onDidStartTerminalShellExecution] Shell execution started, but not from a Claw-registered terminal:",
							e,
						)
					}
				},
			...
			const endDisposable = vscode.window.onDidEndTerminalShellExecution?.(
				async (e: vscode.TerminalShellExecutionEndEvent) => {
					...
					if (!terminal) {
						console.error(
							"[onDidEndTerminalShellExecution] Shell execution ended, but not from a Claw-registered terminal:",
							e,
						)
	...
	public static createTerminal(cwd: string, provider: ClawTerminalProvider): ClawTerminal {
		let newTerminal
	...
	public static async getOrCreateTerminal(
		cwd: string,
		taskId?: string,
		provider: ClawTerminalProvider = "vscode",
	): Promise<ClawTerminal> {
		const terminals = this.getAllTerminals()
		let terminal: ClawTerminal | undefined
	 ...
	 */
	public static getTerminals(busy: boolean, taskId?: string): ClawTerminal[] {
	 ...
	 * @returns Array of Terminal objects
	 */
	public static getBackgroundTerminals(busy?: boolean): ClawTerminal[] {
		return this.getAllTerminals().filter((t) => {
	...
	private static getAllTerminals(): ClawTerminal[] {
		this.terminals = this.terminals.filter((t) => !t.isClosed())
		return this.terminals
	}

	private static getTerminalById(id: number): ClawTerminal | undefined {
	 ...
	 */
	private static getTerminalByVSCETerminal(vsceTerminal: vscode.Terminal): ClawTerminal | undefined {
