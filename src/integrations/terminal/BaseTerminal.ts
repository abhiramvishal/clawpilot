...
import type {
	ClawTerminalProvider,
	ClawTerminal,
	ClawTerminalCallbacks,
	ClawTerminalProcess,
	ClawTerminalProcessResultPromise,
	ExitCodeDetails,
} from "./types"

export abstract class BaseTerminal implements ClawTerminal {
	public readonly provider: ClawTerminalProvider
	...
	public process?: ClawTerminalProcess
	public completedProcesses: ClawTerminalProcess[] = []

	constructor(provider: ClawTerminalProvider, id: number, cwd: string) {
	...
	abstract runCommand(command: string, callbacks: ClawTerminalCallbacks): ClawTerminalProcessResultPromise
	...
	public setActiveStream(stream: AsyncIterable<string> | undefined, pid?: number): void {
		if (stream) {
			if (!this.process) {
				...
				console.warn(
					`[Terminal ${this.provider}/${this.id}] process is undefined, so cannot set terminal stream (probably user-initiated non-Claw command)`,
				)
	 ...
	 */
	public getProcessesWithOutput(): ClawTerminalProcess[] {
