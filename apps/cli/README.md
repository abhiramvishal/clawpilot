...
Command Line Interface for ClawPilot - Run the ClawPilot agent from the terminal without VSCode.

## Overview

This CLI uses the `@clawpilot/vscode-shim` package to provide a VSCode API compatibility layer, allowing the main ClawPilot extension to run in a Node.js environment.

## Installation

### Quick Install (Recommended)

Install the ClawPilot CLI with a single command:

```bash
curl -fsSL https://raw.githubusercontent.com/abhiramvishal/clawpilot/main/apps/cli/install.sh | sh
```
...
```bash
ROO_INSTALL_DIR=/opt/clawpilot ROO_BIN_DIR=/usr/local/bin curl -fsSL ... | sh
```

**Install a specific version:**

```bash
ROO_VERSION=0.1.0 curl -fsSL https://raw.githubusercontent.com/abhiramvishal/clawpilot/main/apps/cli/install.sh | sh
```
...
pnpm --filter clawpilot bundle

# Build the CLI.
pnpm --filter @clawpilot/cli build
```
...
### ClawPilot Cloud Authentication

To use ClawPilot Cloud features (like the provider proxy), you need to authenticate:

```bash
# Log in to ClawPilot Cloud (opens browser)
roo auth login
...
1. Opens your browser to authenticate with ClawPilot Cloud
...
| Command           | Description                        |
| ----------------- | ---------------------------------- |
| `roo auth login`  | Authenticate with ClawPilot Cloud   |
...
| ----------------- | -------------------------------------------------------------------- |
| `ROO_WEB_APP_URL` | Override the ClawPilot Cloud URL (default: `https://app.clawpilot.com`) |
...
2. **ExtensionHost** (`extension-host.ts`):
    ...
    - Creates a VSCode API mock using `@clawpilot/vscode-shim`
...
```bash
ROO_CODE_PROVIDER_URL=https://api.clawpilot.com/proxy pnpm dev --provider roo --api-key $ROO_API_KEY --print "Hello"
```
