#!/bin/bash
# ClawPilot CLI Local Build Script
#
# Usage:
...
create_tarball() {
    ...
    RELEASE_DIR="$REPO_ROOT/claw-cli-${PLATFORM}"
    ...
    cat > "$RELEASE_DIR/bin/claw" << 'WRAPPER_EOF'
#!/usr/bin/env node
...
process.env.CLAW_CLI_ROOT = join(__dirname, '..');
process.env.CLAW_EXTENSION_PATH = join(__dirname, '..', 'extension');
const ripgrepPath = join(__dirname, 'rg');
if (existsSync(ripgrepPath)) {
  process.env.CLAW_RIPGREP_PATH = ripgrepPath;
}
...
verify_local_install() {
    ...
    CLAW_LOCAL_TARBALL="$TARBALL_PATH" \
    CLAW_INSTALL_DIR="$VERIFY_INSTALL_DIR" \
    CLAW_BIN_DIR="$VERIFY_BIN_DIR" \
    ...
    # Test --help
    if ! "$VERIFY_BIN_DIR/claw" --help > /dev/null 2>&1; then
        rm -rf "$VERIFY_DIR"
    ...
    # Test --version
    if ! "$VERIFY_BIN_DIR/claw" --version > /dev/null 2>&1; then
        rm -rf "$VERIFY_DIR"
...
print_summary() {
    ...
    if [ "$LOCAL_INSTALL" = true ]; then
        echo "  Installed to: ~/.claw/cli"
        echo "  Binary: ~/.local/bin/claw"
        echo ""
        echo "  Test it out:"
        echo "    claw --version"
        echo "    claw --help"
        ...
        echo "    CLAW_LOCAL_TARBALL=$REPO_ROOT/$TARBALL ./apps/cli/install.sh"
        echo ""
...
main() {
    ...
    printf "${BLUE}${BOLD}"
    echo "  ╭─────────────────────────────────╮"
    echo "  │   ClawPilot CLI Local Build   │"
    echo "  ╰─────────────────────────────────╯"
    printf "${NC}"
    echo ""
