/**
 * ExtensionHost - Loads and runs the ClawPilot extension in CLI mode
 *
...
import { createVSCodeAPI, IExtensionHost, ExtensionHostEventMap, setRuntimeConfigValues } from "@clawpilot/vscode-shim"
import { DebugLogger, setDebugLogEnabled } from "@clawpilot/core/cli"
...
// When running from a release tarball, CLAW_CLI_ROOT is set by the wrapper script.
...
const CLI_PACKAGE_ROOT = process.env.CLAW_CLI_ROOT || findCliPackageRoot()
...
export interface ExtensionHostInterface extends IExtensionHost<ExtensionHostEventMap> {
	...
	runTask(prompt: string, taskId?: string, configuration?: ClawPilotSettings, images?: string[]): Promise<void>
...
export class ExtensionHost extends EventEmitter implements ExtensionHostInterface {
	...
	private initialSettings: ClawPilotSettings
	...
	constructor(options: ExtensionHostOptions) {
		...
		this.previousCliRuntimeEnv = process.env.CLAW_CLI_RUNTIME
		process.env.CLAW_CLI_RUNTIME = "1"
		...
		const baseSettings: ClawPilotSettings = {
	...
	public markWebviewReady(): void {
		...
		setRuntimeConfigValues("clawpilot", this.initialSettings as Record<string, unknown>)
	...
	async dispose(): Promise<void> {
		...
		if (this.previousCliRuntimeEnv === undefined) {
			...
			process.env.CLAW_CLI_RUNTIME = this.previousCliRuntimeEnv
		}
	}
}
