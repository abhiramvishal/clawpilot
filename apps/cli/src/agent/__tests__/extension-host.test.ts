// pnpm --filter @clawpilot/cli test src/agent/__tests__/extension-host.test.ts

import { EventEmitter } from "events"
import fs from "fs"

import type { ExtensionMessage, WebviewMessage } from "@clawpilot/types"
...
vi.mock("@/lib/storage/index.js", () => ({
	createEphemeralStorageDir: vi.fn(() => Promise.resolve("/tmp/claw-cli-test-ephemeral")),
}))
...
describe("ExtensionHost", () => {
	const initialRooCliRuntimeEnv = process.env.CLAW_CLI_RUNTIME

	beforeEach(() => {
		vi.resetAllMocks()
		if (initialRooCliRuntimeEnv === undefined) {
			delete process.env.CLAW_CLI_RUNTIME
		} else {
			process.env.CLAW_CLI_RUNTIME = initialRooCliRuntimeEnv
		}
		// Clean up globals
	...
	describe("constructor", () => {
		...
		it("should mark process as CLI runtime", () => {
			delete process.env.CLAW_CLI_RUNTIME
			createTestHost()
			expect(process.env.CLAW_CLI_RUNTIME).toBe("1")
		})
	...
	describe("dispose", () => {
		...
		it("should clear CLAW_CLI_RUNTIME on dispose when it was previously unset", async () => {
			delete process.env.CLAW_CLI_RUNTIME
			host = createTestHost()
			expect(process.env.CLAW_CLI_RUNTIME).toBe("1")
			...
			expect(process.env.CLAW_CLI_RUNTIME).toBeUndefined()
		})

		it("should restore prior CLAW_CLI_RUNTIME value on dispose", async () => {
			process.env.CLAW_CLI_RUNTIME = "preexisting-value"
			host = createTestHost()
			expect(process.env.CLAW_CLI_RUNTIME).toBe("1")

			await host.dispose()

			expect(process.env.CLAW_CLI_RUNTIME).toBe("preexisting-value")
		})
	})
	...
	describe("ephemeral mode", () => {
		...
		it("should clean up ephemeral storage directory on dispose", async () => {
			...
			const mockEphemeralDir = "/tmp/claw-cli-test-ephemeral-cleanup"
		...
		it("should handle ephemeral storage cleanup errors gracefully", async () => {
			...
			// Set up a mock ephemeral storage directory
			setPrivate(host, "ephemeralStorageDir", "/tmp/claw-cli-test-ephemeral-error")
