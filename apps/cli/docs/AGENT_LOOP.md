...
This document explains how the ClawPilot CLI detects and tracks the agent loop state.
...
The CLI uses type guards from `@clawpilot/types` for categorization:

```typescript
import { isIdleAsk, isInteractiveAsk, isResumableAsk, isNonBlockingAsk } from "@clawpilot/types"
