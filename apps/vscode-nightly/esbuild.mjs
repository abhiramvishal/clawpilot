...
import { getGitSha, copyPaths, copyLocales, copyWasms, generatePackageJson } from "@clawpilot/build"
...
async function main() {
	...
	const buildOptions = {
		...
		platform: "node",
		define: {
			"process.env.PKG_NAME": '"clawpilot-nightly"',
			"process.env.PKG_VERSION": `"${overrideJson.version}"`,
			"process.env.PKG_OUTPUT_CHANNEL": '"clawpilot-Nightly"',
