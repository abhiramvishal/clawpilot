...
import { type TokenUsage, type ToolUsage, ClawPilotEventName, taskEventSchema } from "@clawpilot/types"
import type { Run, Task, TaskMetrics } from "@clawpilot/evals"
...
export const useRunStatus = (run: Run): RunStatus => {
	...
	const onMessage = useCallback((messageEvent: MessageEvent) => {
		...
		switch (eventName) {
			case ClawPilotEventName.TaskStarted:
				startTimes.current.set(taskId, Date.now())
				break
			case ClawPilotEventName.TaskTokenUsageUpdated: {
			...
			case ClawPilotEventName.EvalPass:
			case ClawPilotEventName.EvalFail:
