...
export async function createRun({
	...
	try {
		...
		const cliCommand = `pnpm --filter @clawpilot/evals cli --runId ${run.id}`
		...
		const logStream = fs.createWriteStream("/tmp/clawpilot-evals.log", { flags: "a" })
...
export async function deleteOldRuns(): Promise<DeleteIncompleteRunsResult> {
	...
	const { getRuns } = await import("@clawpilot/evals")
