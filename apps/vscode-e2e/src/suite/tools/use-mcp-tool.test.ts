...
import { ClawPilotEventName, type ClineMessage } from "@clawpilot/types"

import { waitFor, sleep } from "../utils"
import { setDefaultSuiteTimeout } from "../test-utils"

suite.skip("ClawPilot use_mcp_tool Tool", function () {
	...
	test("Should request MCP filesystem read_file tool and complete successfully", async function () {
		...
		api.on(ClawPilotEventName.Message, messageHandler)
		...
		api.on(ClawPilotEventName.TaskStarted, taskStartedHandler)
		...
		api.on(ClawPilotEventName.TaskCompleted, taskCompletedHandler)
		await sleep(2000) // Wait for ClawPilot to fully initialize
		...
		try {
			...
			// Make a small modification to trigger the save event, without this ClawPilot won't load the MCP server
		...
		try {
			...
			// Clean up
			api.off(ClawPilotEventName.Message, messageHandler)
			api.off(ClawPilotEventName.TaskStarted, taskStartedHandler)
			api.off(ClawPilotEventName.TaskCompleted, taskCompletedHandler)
		}
	})
