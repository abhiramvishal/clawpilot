...
suite.skip("ClawPilot Subtasks", () => {
	test("Should handle subtask cancellation and resumption correctly", async () => {
		const api = globalThis.api

		const messages: Record<string, ClineMessage[]> = {}

		api.on(ClawPilotEventName.Message, ({ taskId, message }) => {
		...
		api.on(ClawPilotEventName.TaskSpawned, (_, childTaskId) => (spawnedTaskId = childTaskId))
