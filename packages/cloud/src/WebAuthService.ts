...
import { getClerkBaseUrl, getClawPilotApiUrl, PRODUCTION_CLERK_BASE_URL } from "./config.js"
...
export class WebAuthService extends EventEmitter<AuthServiceEvents> implements AuthService {
	...
	public async login(landingPageSlug?: string, useProviderSignup: boolean = false): Promise<void> {
		try {
			...
			const publisher = packageJSON?.publisher ?? "clawpilot"
			const name = packageJSON?.name ?? "clawpilot"
			...
			const url = landingPageSlug
				? `${getClawPilotApiUrl()}/l/${landingPageSlug}?${params.toString()}`
				: useProviderSignup
					? `${getClawPilotApiUrl()}/extension/provider-sign-up?${params.toString()}`
					: `${getClawPilotApiUrl()}/extension/sign-in?${params.toString()}`

			await vscode.env.openExternal(vscode.Uri.parse(url))
		} catch (error) {
			const context = landingPageSlug ? ` (landing page: ${landingPageSlug})` : ""
			this.log(`[auth] Error initiating ClawPilot Cloud auth${context}: ${error}`)
			throw new Error(`Failed to initiate ClawPilot Cloud authentication${context}: ${error}`)
		}
	}

	/**
	 * Handle the callback from ClawPilot Cloud
	 *
	 * This method is called when the user is redirected back to the extension
	 * after authenticating with ClawPilot Cloud.
	 *
	...
	public async handleCallback(
		...
		if (!code || !state) {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Invalid ClawPilot Cloud sign in url")
			}
		...
		try {
			...
			if (providerModel) {
				await this.context.globalState.update("claw-provider-model", providerModel)
				await this.context.globalState.update("claw-auth-skip-model", undefined)
				this.log(`[auth] Stored provider model: ${providerModel}`)
			} else {
				// No model was selected during signup - flag this for the webview
				await this.context.globalState.update("claw-auth-skip-model", true)
			...
			if (vscode) {
				vscode.window.showInformationMessage("Successfully authenticated with ClawPilot Cloud")
			}

			this.log("[auth] Successfully authenticated with ClawPilot Cloud")
		} catch (error) {
			this.log(`[auth] Error handling ClawPilot Cloud callback: ${error}`)
			this.changeState("logged-out")
			throw new Error(`Failed to handle ClawPilot Cloud callback: ${error}`)
		}
	}
	...
	public async logout(): Promise<void> {
		...
		try {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Logged out from ClawPilot Cloud")
			}

			this.log("[auth] Logged out from ClawPilot Cloud")
		} catch (error) {
			this.log(`[auth] Error logging out from ClawPilot Cloud: ${error}`)
			throw new Error(`Failed to log out from ClawPilot Cloud: ${error}`)
		}
	}
