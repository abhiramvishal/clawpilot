...
import type { SettingsService, AuthService } from "@clawpilot/types"
...
vi.mock("../Config", () => ({
	getClawPilotApiUrl: () => "https://app.clawpilot.com",
}))

vi.mock("../utils", () => ({
	getUserAgent: () => "clawpilot 1.0.0",
}))
	...
	describe("shareTask", () => {
		it("should share task with organization visibility and copy to clipboard", async () => {
			const mockResponseData = {
				success: true,
				shareUrl: "https://app.clawpilot.com/share/abc123",
			}
			...
			expect(result.shareUrl).toBe("https://app.clawpilot.com/share/abc123")

			expect(mockFetch).toHaveBeenCalledWith("https://app.clawpilot.com/api/extension/share", {
				method: "POST",
				headers: {
					"Content-Type": "application/json",
					Authorization: "Bearer session-token",
					"User-Agent": "clawpilot 1.0.0",
				},
				body: JSON.stringify({
					taskId: "task-123",
			...
			expect(vscode.env.clipboard.writeText).toHaveBeenCalledWith("https://app.clawpilot.com/share/abc123")
		})
