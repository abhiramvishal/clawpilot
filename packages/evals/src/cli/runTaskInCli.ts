...
import { type ToolUsage, TaskCommandName, ClawPilotEventName, IpcMessageType } from "@clawpilot/types"
...
/**
 * Run a task using the ClawPilot CLI (headless mode).
...
export const runTaskWithCli = async ({ run, task, publish, logger, jobToken }: RunTaskOptions) => {
	...
	const ignoreEventsForBroadcast = [ClawPilotEventName.Message]
