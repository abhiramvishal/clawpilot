...
import { IpcClient } from "@clawpilot/ipc"
...
export const runTaskInVscode = async ({ run, task, publish, logger, jobToken }: RunTaskOptions) => {
	...
	const ignoreEvents: Record<"broadcast" | "log", ClawPilotEventName[]> = {
		broadcast: [ClawPilotEventName.Message],
		log: [ClawPilotEventName.TaskTokenUsageUpdated, ClawPilotEventName.TaskAskResponded],
	}
	...
	client.on(IpcMessageType.TaskEvent, async (taskEvent) => {
		...
		if (
			eventName === ClawPilotEventName.Message &&
		...
		if (
			...
			if (eventName === ClawPilotEventName.Message) {
			...
			if (eventName === ClawPilotEventName.Message && payload[0]?.message?.ask === "tool") {
				try {
			...
			} else if (eventName === ClawPilotEventName.Message && payload[0]?.message?.ask === "command") {
				logEventName = `${eventName} (command)`
			} else if (eventName === ClawPilotEventName.Message && payload[0]?.message?.ask === "completion_result") {
		...
		if (eventName === ClawPilotEventName.TaskTokenUsageUpdated || eventName === ClawPilotEventName.TaskCompleted) {
		...
		if (eventName === ClawPilotEventName.TaskAborted) {
			taskAbortedAt = Date.now()
		}

		if (eventName === ClawPilotEventName.TaskCompleted) {
