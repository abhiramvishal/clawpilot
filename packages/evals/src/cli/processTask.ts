...
import { type TaskEvent, ClawPilotEventName } from "@clawpilot/types"
...
export const processTask = async ({
	...
	try {
		...
		await publish({
			eventName: passed ? ClawPilotEventName.EvalPass : ClawPilotEventName.EvalFail,
			taskId: task.id,
		})
	} finally {
...
export const processTaskInContainer = async ({
	...
	if (jobToken) {
		baseArgs.push(`-e CLAW_PILOT_CLOUD_TOKEN=${jobToken}`)
	}
	...
	const command = `pnpm --filter @clawpilot/evals cli --taskId ${taskId}`
	logger.info(command)
