			...
			handler: async (args) => {
				...
				try {
					if (ci) {
						...
						await processTask({ taskId, jobToken: process.env.CLAW_PILOT_CLOUD_TOKEN || null })
					} else {
...
if (!fs.existsSync(EVALS_REPO_PATH)) {
	console.error(
		`Exercises do not exist at ${EVALS_REPO_PATH}. Please run "git clone https://github.com/abhiramvishal/clawpilot-evals.git evals".`,
	)
