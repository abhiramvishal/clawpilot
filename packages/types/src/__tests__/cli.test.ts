import {
	clawCliControlEventSchema,
	clawCliFinalOutputSchema,
	clawCliInputCommandSchema,
	clawCliStreamEventSchema,
} from "../cli.js"

describe("CLI types", () => {
	describe("clawCliInputCommandSchema", () => {
		it("validates a start command", () => {
			const result = clawCliInputCommandSchema.safeParse({
				command: "start",
				requestId: "req-1",
				prompt: "hello",
	...
	describe("clawCliControlEventSchema", () => {
		it("validates a control done event", () => {
			const result = clawCliControlEventSchema.safeParse({
				type: "control",
				subtype: "done",
				requestId: "req-3",
				command: "start",
	...
	describe("clawCliStreamEventSchema", () => {
		it("accepts passthrough fields for forward compatibility", () => {
			const result = clawCliStreamEventSchema.safeParse({
				type: "assistant",
				id: 42,
				content: "partial",
				customField: "future",
	...
	describe("clawCliFinalOutputSchema", () => {
		it("validates final json output shape", () => {
			const result = clawCliFinalOutputSchema.safeParse({
				type: "result",
				success: true,
				content: "done",
				events: [],
