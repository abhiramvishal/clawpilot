...
describe("MockWorkspaceConfiguration", () => {
	...
	describe("Runtime Configuration", () => {
		...
		it("should return runtime config value over disk-based values", async () => {
			const config = new MockWorkspaceConfiguration("clawpilot", context)
			...
			setRuntimeConfig("clawpilot", "commandExecutionTimeout", 20)
		...
		it("should set and get runtime config values", () => {
			setRuntimeConfig("clawpilot", "testSetting", "testValue")

			expect(getRuntimeConfig("clawpilot.testSetting")).toBe("testValue")
		})

		it("should set multiple runtime config values at once", () => {
			setRuntimeConfigValues("clawpilot", {
				setting1: "value1",
				setting2: 42,
				setting3: true,
			})

			expect(getRuntimeConfig("clawpilot.setting1")).toBe("value1")
			expect(getRuntimeConfig("clawpilot.setting2")).toBe(42)
			expect(getRuntimeConfig("clawpilot.setting3")).toBe(true)
		})
		...
		it("should ignore undefined values in setRuntimeConfigValues", () => {
			...
			expect(getRuntimeConfig("clawpilot.defined")).toBe("value")
			expect(getRuntimeConfig("clawpilot.notDefined")).toBeUndefined()
		})

		it("should clear all runtime config values", () => {
			setRuntimeConfig("clawpilot", "setting1", "value1")
			setRuntimeConfig("clawpilot", "setting2", "value2")

			clearRuntimeConfig()

			expect(getRuntimeConfig("clawpilot.setting1")).toBeUndefined()
			expect(getRuntimeConfig("clawpilot.setting2")).toBeUndefined()
		})
