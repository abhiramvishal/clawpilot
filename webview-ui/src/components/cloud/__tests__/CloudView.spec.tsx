	...
	useAppTranslation: () => ({
		t: (key: string) => {
			const translations: Record<string, string> = {
				...
				"cloud:visitCloudWebsite": "Visit ClawPilot Cloud",
				"cloud:taskSync": "Task sync",
				"cloud:taskSyncDescription": "Sync your tasks for viewing and sharing on ClawPilot Cloud",
				...
				"cloud:cloudUrlPillLabel": "ClawPilot Cloud URL: ",
			}
...
describe("CloudView", () => {
	it("should display benefits when user is not authenticated", () => {
		render(<CloudView userInfo={null} isAuthenticated={false} cloudApiUrl="https://app.clawpilot.com" />)
	...
	it("should not display cloud URL pill when pointing to production", () => {
		...
		expect(screen.queryByText(/ClawPilot Cloud URL:/)).not.toBeInTheDocument()
	})
	...
	it("should display cloud URL pill when pointing to non-production environment", () => {
		...
		render(<CloudView userInfo={mockUserInfo} isAuthenticated={true} cloudApiUrl="https://staging.clawpilot.com" />)

		// Check that the cloud URL pill is displayed with the staging URL
		expect(screen.getByText(/ClawPilot Cloud URL:/)).toBeInTheDocument()
		expect(screen.getByText("https://staging.clawpilot.com")).toBeInTheDocument()
	})

	it("should display cloud URL pill for non-authenticated users when not pointing to production", () => {
		render(<CloudView userInfo={null} isAuthenticated={false} cloudApiUrl="https://dev.clawpilot.com" />)

		// Check that the cloud URL pill is displayed even when not authenticated
		expect(screen.getByText(/ClawPilot Cloud URL:/)).toBeInTheDocument()
		expect(screen.getByText("https://dev.clawpilot.com")).toBeInTheDocument()
	})
