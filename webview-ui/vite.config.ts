...
export default defineConfig(({ mode }) => {
	...
	const define: Record<string, any> = {
		...
		"process.env.PKG_OUTPUT_CHANNEL": JSON.stringify("clawpilot"),
		...(gitSha ? { "process.env.PKG_SHA": JSON.stringify(gitSha) } : {}),
	}

	// TODO: We can use `@clawpilot/build` to generate `define` once the
	...
	if (mode === "nightly") {
		...
		define["process.env.PKG_OUTPUT_CHANNEL"] = JSON.stringify("clawpilot-Nightly")
	}
	...
	return {
		...
		resolve: {
			alias: {
				...
				"@claw": resolve(__dirname, "../src/shared"),
			},
		},
		build: {
			outDir,
			...
			rollupOptions: {
				...
				// dynamically imported by claw-config/index.ts, but should never be bundled
